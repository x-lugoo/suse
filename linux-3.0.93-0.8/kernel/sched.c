...
#if defined(CONFIG_IA64) || defined(CONFIG_KGDB) || defined(CONFIG_KDB)
/*
...
#endif /* defined(CONFIG_IA64) || defined(CONFIG_KGDB) */
...
static void
kdb_prio(char *name, struct rt_prio_array *array, lkdb_printf_t xxx_printf,
	unsigned int cpu)
{
...
void
lkdb_runqueue(unsigned long cpu, lkdb_printf_t xxx_printf)
{
	int i;
	struct rq *rq;
	...
	xxx_printf("CPU%ld lock:%s curr:0x%p(%d)(%s)",
		   cpu, (spin_is_locked((spinlock_t *)&rq->lock))?"LOCKED":"free",
