 ...
 * kgr_shadow_*alloc() API calls, it is considered live: any subsequent
 * call to kgr_shadow_get() may then return the shadow variable's data
 * pointer.  Callers of kgr_shadow_*alloc() should prepare shadow data
 * accordingly.
 *
 * The kgr_shadow_*alloc() API calls may allocate memory for new shadow
 ...
 * concurrent kgr_shadow_free() and kgr_shadow_get() operations.
 */
...
static DEFINE_HASHTABLE(kgr_shadow_hash, 12);

/*
 * kgr_shadow_lock provides exclusive access to the kgr_shadow_hash and
 * the shadow variables it references.
 */
static DEFINE_SPINLOCK(kgr_shadow_lock);

/**
 * struct kgr_shadow - shadow variable structure
...
/**
 * kgr_shadow_match() - verify a shadow variable matches given <obj, id>
 ...
 */
static inline bool kgr_shadow_match(struct kgr_shadow *shadow, void *obj,
				unsigned long id)
{
...
/**
 * kgr_shadow_get() - retrieve a shadow variable data pointer
 * @obj:	pointer to parent object
 * @id:		data identifier
 *
 * Return: the shadow variable data element, NULL on failure.
 */
void *kgr_shadow_get(void *obj, unsigned long id)
{
...
EXPORT_SYMBOL_GPL(kgr_shadow_free);
...
EXPORT_SYMBOL_GPL(kgr_shadow_free_all);
